...
print("="*60)
print("CAMERA MODE - HAND KEYPOINT TRACKING")
print(f"  Model: handkeypoint.pt")
...
print("="*60)
print("MULTITHREADING MODE - HAND KEYPOINT TRACKING")
print("  Thread 1: Frame Grabber (đọc frames từ stream)")
print("  Thread 2: Hand Keypoint Tracking (tracking với YOLO built-in)")
print("  Main Thread: Display (hiển thị kết quả với keypoints)")
...
if SHOW_VIDEO:
    cv2.namedWindow("Hand Keypoint Tracking", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Hand Keypoint Tracking", 1280, 720)
    ...
    try:
        ...
        if SHOW_VIDEO:
            ...
            if result and hasattr(result, 'boxes') and result.boxes is not None and len(result.boxes) > 0:
                try:
                    ...
                    for i, (box, conf, cls) in enumerate(zip(boxes, confidences, classes)):
                        ...
                        # Vẽ keypoints nếu có (hand keypoint model)
            ...
            if annotated_frame is not None and current_fps is not None and avg_fps_display is not None:
                ...
                cv2.imshow("Hand Keypoint Tracking", annotated_frame)
...
print(f"\n{'='*60}")
print(f"REALTIME SUMMARY - HAND KEYPOINT TRACKING:")
print(f"  Model: handkeypoint.pt")
