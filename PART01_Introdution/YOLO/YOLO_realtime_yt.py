...
SOURCE = "https://www.youtube.com/watch?v=cH7VBI4QQzA"  # Thay đổi URL này thành YouTube URL bạn muốn
...
# Xử lý YouTube source
target_fps = 30.0  # Default FPS (FPS lý thuyết/mong đợi từ YouTube - có thể không khớp với thực tế)
...
def frame_grabber_thread():
    ...
    # Tối ưu cho YouTube streams (HLS/Direct): giảm buffer để giảm latency
...
print(f"Starting inference with YouTube stream: {stream_url_str[:80]}{'...' if len(stream_url_str) > 80 else ''}")
...
while not stop_flag.is_set():
    try:
        ...
        # Đo Input FPS thực tế: FPS thực sự đọc được từ YouTube stream (khác với Target FPS lý thuyết)
        # Input FPS = 1 / (thời gian giữa 2 frames liên tiếp)
        # Ví dụ: Target FPS = 30 (YouTube báo), nhưng Input FPS = 28 (thực tế đọc được)
