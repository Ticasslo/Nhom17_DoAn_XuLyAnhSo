...
if torch.cuda.is_available():
    ...
    model_name = "handkeypoint.pt"
else:
    model_name = "handkeypoint.pt" # CPU mode
...
INFERENCE_PARAMS = {
    "imgsz": imgsz,
    "conf": 0.3,  # Confidence threshold (giảm từ 0.4 → 0.3: detect nhiều hơn khi di chuyển nhanh, motion blur)
...
# ---------- Tracker Setup ----------
TRACKER_CONFIG = {
    "track_activation_threshold": 0.15,     # Confidence threshold để tạo track mới (giảm từ default 0.25 → 0.15: capture nhiều detection hơn, kể cả confidence thấp khi di chuyển nhanh)
    "lost_track_buffer": 200,               # Số frames giữ track khi mất detection (tăng từ default 30 → 200: giữ track lâu hơn khi di chuyển nhanh/mất detection tạm thời)
    "minimum_matching_threshold": 0.5,      # IoU threshold để match tracks (giảm từ default 0.8 → 0.5: cho phép match với IoU thấp hơn khi di chuyển nhanh từ vị trí này sang vị trí khác)
...
def draw_keypoints(frame, keypoints, color=(0, 255, 255), radius=3, conf_threshold=0.3):
    ...
    for kp in keypoints:
        if len(kp) >= 2:
            ...
            # Vẽ keypoint nếu confidence đủ và tọa độ hợp lệ (> 0)
            if conf >= conf_threshold and x > 0 and y > 0:
                x, y = int(x), int(y)
                # Vẽ điểm keypoint với viền trắng mỏng để dễ nhìn
                cv2.circle(frame, (x, y), radius + 1, (255, 255, 255), -1)  # Viền trắng
...
def draw_hand_skeleton(frame, keypoints, color=(0, 255, 255), thickness=1, conf_threshold=0.3):
    ...
    Cấu trúc 21 keypoints:
    - 0: Wrist (cổ tay)
    - 1-4: Thumb (ngón cái): 1=MCP, 2=IP, 3=Tip
    ...
    # Hand keypoint connections theo cấu trúc chuẩn
    # Wrist to finger bases
    wrist_to_fingers = [(0, 1), (0, 5), (0, 9), (0, 13), (0, 17)]
    
    # Thumb: MCP -> IP -> Tip (lưu ý: thumb có 4 keypoints nhưng chỉ 3 joints)
    ...
    for start_idx, end_idx in all_connections:
        if start_idx < len(keypoints) and end_idx < len(keypoints):
            ...
            if len(kp1) >= 2 and len(kp2) >= 2:
                ...
                # Vẽ connection nếu cả 2 keypoints đều có confidence đủ và tọa độ hợp lệ
                ...
                try:
                    ...
                    for i in range(len(boxes)):
                        try:
                            ...
                            # Vẽ keypoints nếu có (từ result, không phải từ tracks vì tracks không có keypoints)
                            if result and hasattr(result, 'keypoints') and result.keypoints is not None:
                                try:
                                    keypoints_cpu = result.keypoints.cpu()
                                    # Kiểm tra số lượng detections có keypoints
                                    if hasattr(keypoints_cpu, 'data') and len(keypoints_cpu.data) > i:
                                        # Lấy keypoints cho detection thứ i
                                        # Shape: (num_keypoints, 3) với (x, y, confidence)
                                        kp_data = keypoints_cpu.data[i].numpy()
                                        if len(kp_data) > 0:
                                            # Vẽ skeleton connections trước (để keypoints nằm trên)
                                            if len(kp_data) == 21:  # Hand model có 21 keypoints
                                                draw_hand_skeleton(annotated_frame, kp_data, color, 1, conf_threshold=0.3)
                                            # Vẽ keypoints
                                            draw_keypoints(annotated_frame, kp_data, color, 3, conf_threshold=0.3)
                                    elif hasattr(keypoints_cpu, 'xy') and len(keypoints_cpu.xy) > i:
                                        # Fallback: dùng xy nếu không có data
                                        kp_xy = keypoints_cpu.xy[i].numpy()  # Shape: (num_keypoints, 2)
                                        if len(kp_xy) > 0:
                                            # Tạo array với confidence = 1.0 (giả định)
                                            kp_data = np.column_stack([kp_xy, np.ones(len(kp_xy))])
                                            if len(kp_data) == 21:
                                                draw_hand_skeleton(annotated_frame, kp_data, color, 1, conf_threshold=0.3)
                                            draw_keypoints(annotated_frame, kp_data, color, 3, conf_threshold=0.3)
                                except Exception as e:
                                    pass  # Bỏ qua nếu không vẽ được keypoints
                        except Exception as e:
            ...
            # Fallback: nếu không có tracks, vẽ detections thô từ YOLO (không có track ID)
                ...
                try:
                    ...
                    for idx, (box, conf, cls) in enumerate(zip(boxes, confidences, classes)):
                        ...
                        color = (0, 255, 0)
                        draw_box_with_label(annotated_frame, x1, y1, x2, y2, label, color, 0.6)
                        ...
                        if hasattr(result, 'keypoints') and result.keypoints is not None:
                            try:
                                keypoints_cpu = result.keypoints.cpu()
                                # Kiểm tra số lượng detections có keypoints
                                if hasattr(keypoints_cpu, 'data') and len(keypoints_cpu.data) > idx:
                                    # Lấy keypoints cho detection thứ idx
                                    # Shape: (num_keypoints, 3) với (x, y, confidence)
                                    kp_data = keypoints_cpu.data[idx].numpy()
                                    if len(kp_data) > 0:
                                        # Vẽ skeleton connections trước (để keypoints nằm trên)
                                        if len(kp_data) == 21:  # Hand model có 21 keypoints
                                            draw_hand_skeleton(annotated_frame, kp_data, color, 1, conf_threshold=0.3)
                                        # Vẽ keypoints
                                        draw_keypoints(annotated_frame, kp_data, color, 3, conf_threshold=0.3)
                                elif hasattr(keypoints_cpu, 'xy') and len(keypoints_cpu.xy) > idx:
                                    # Fallback: dùng xy nếu không có data
                                    kp_xy = keypoints_cpu.xy[idx].numpy()  # Shape: (num_keypoints, 2)
                                    if len(kp_xy) > 0:
                                        # Tạo array với confidence = 1.0 (giả định)
                                        kp_data = np.column_stack([kp_xy, np.ones(len(kp_xy))])
                                        if len(kp_data) == 21:
